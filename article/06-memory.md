...
**作用**：Agent 的 `chat()` 方法会自动调用 `MemoryManager.remember()`，无需手动干预。

### 触发时机

| 触发点 | 记录内容 | 类型 | 重要度 | 标签 |
|--------|----------|------|--------|------|
| 用户输入 | 用户的原始输入文本（跳过短输入和 `/` 命令） | `EPISODIC` | 0.3 | `["user_input"]` |
| 助手回复 | 助手文本回复的前 500 字符（跳过 20 字以下的短回复） | `EPISODIC` | 0.4 | `["assistant_response"]` |
| 工具调用序列 | 单次 chat 中连续调用的工具序列（≥3 步时记录） | `PROCEDURAL` | 0.5 | `["workflow", "tool_pattern"]` |

### 工具序列去重

对于程序性记忆（工具调用序列），系统会自动去重：如果相同的工具序列模式已存在于记忆中，不会重复写入，而是递增 `metadata.access_count`，便于后续 dream 阶段识别高频工作流。

### 自动 Dream 触发

每次 `chat()` 返回前，系统会检查 `AutoTrigger.should_dream()` 条件：
- 记忆总数 > 50 条
- 距离上次 dream 已超过 24 小时

条件满足时静默执行 dream 巩固（矛盾检测、合并、强化、归档）。
